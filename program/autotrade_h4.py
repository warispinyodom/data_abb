...
TIMEFRAME_TREND = mt5.TIMEFRAME_H4   # เปลี่ยนเป็น Timeframe H4 สำหรับเช็คเทรนด์ใหญ่
...
# ==========================================
def ai_probability_analysis(df_entry, trend_h4):
  ...
  signal = 'NONE'
  if probability >= 65.0:
    signal = 'BUY' if trend_h4 == 'BULLISH' else 'SELL'
...
def print_ui_dashboard(status_data):
  ...
  print(
      f" Trend H4     : {status_data['trend_h4']} (EMA20:"
      f" {status_data['ema20_h4']:.2f} | EMA50: {status_data['ema50_h4']:.2f})"
  )
  print(
...
def main():
  ...
  while True:
    ...
    df_h4 = fetch_data(SYMBOL, TIMEFRAME_TREND)
    df_entry = fetch_data(SYMBOL, TIMEFRAME_ENTRY)

    if df_h4 is None or df_entry is None:
      time.sleep(10)
      continue
    ...
    trend_h4 = (
        'BULLISH' if last_h4['ema20'] > last_h4['ema50'] else 'BEARISH'
    )
    ...
    prob, signal, atr = ai_probability_analysis(df_entry, trend_h4)

    # คำนวณ SL / TP โดย AI ใช้ค่า ATR (RR ~ 1:2)
    current_price = tick.ask if trend_h4 == 'BULLISH' else tick.bid
    sl_distance = atr * 1.5
    tp_distance = atr * 3.0
    ...
    dashboard_data = {
        ...
        'trend_h4': trend_h4,
        'ema20_h4': last_h4['ema20'],
        'ema50_h4': last_h4['ema50'],
        'adx_val': adx_val,
